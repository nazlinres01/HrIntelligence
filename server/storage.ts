...
export class DatabaseStorage implements IStorage {
  ...
  async getActiveUsers(): Promise<any[]> {
    return await db.select().from(users).where(eq(users.isActive, true)).limit(10);
  }
  ...
  async getAuditLogs(limit: number = 100, companyId?: number): Promise<AuditLog[]> {
    if (companyId) {
      return await db.select().from(auditLogs).where(eq(auditLogs.companyId, companyId)).orderBy(desc(auditLogs.createdAt)).limit(limit);
    }
    return await db.select().from(auditLogs).orderBy(desc(auditLogs.createdAt)).limit(limit);
  }
  ...
  async getActivities(limit: number = 50, companyId?: number): Promise<Activity[]> {
    if (companyId) {
      return await db.select().from(activities).where(eq(activities.companyId, companyId)).orderBy(desc(activities.timestamp)).limit(limit);
    }
    return await db.select().from(activities).orderBy(desc(activities.timestamp)).limit(limit);
  }
